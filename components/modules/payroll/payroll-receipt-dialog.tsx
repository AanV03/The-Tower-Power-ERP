...
import type { PayrollReceiptView } from "@/components/modules/payroll/types";
...
function BreakdownRow({
  ...
  return (
    <div className="flex items-center justify-between gap-4 py-2 text-sm">
      ...
      <span className={strong ? "text-base font-semibold text-foreground" : "font-medium text-foreground"}>
        {value}
      </span>
    </div>
  );
    ...
    <Dialog>
      ...
      <DialogContent className="max-h-[calc(100vh-2rem)] overflow-y-auto sm:max-w-2xl">
        <DialogHeader className="pr-8">
          ...
          <DialogTitle className="text-xl">Recibo de nomina</DialogTitle>
          <DialogDescription>
        ...
        <div className="grid gap-4 md:grid-cols-[1fr_220px]">
          ...
          <div className="rounded-lg border border-border bg-muted/20 p-4">
            ...
            <p className="text-sm text-muted-foreground">Estatus: {payrollLabels.status[receipt.status]}</p>
          </div>
        </div>

        <div className="rounded-lg border border-border p-4">
          <BreakdownRow label={payrollLabels.receipts.base} value={receipt.baseLabel} />
          <BreakdownRow label={payrollLabels.receipts.overtime} value={receipt.overtimeLabel} />
          <BreakdownRow label={payrollLabels.receipts.commission} value={receipt.commissionLabel} />
          <BreakdownRow label={payrollLabels.receipts.deductions} value={`-${receipt.deductionsLabel}`} />
        ...
        <div className="rounded-lg border border-dashed border-border p-4 text-sm text-muted-foreground">
          Contrato aplicado, asistencia relacionada y notas de incidencia quedan listos para conectarse cuando exista el flujo de calculo.
        </div>

        <DialogFooter>
          <Button type="button" variant="outline" disabled>
            <Download className="size-4" aria-hidden="true" />
            {payrollLabels.actions.export}
          </Button>
          <Button type="button" variant="outline" disabled>
