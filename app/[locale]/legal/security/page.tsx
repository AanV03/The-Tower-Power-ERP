...
export async function generateMetadata({ params }: LegalPageParams): Promise<Metadata> {
  ...
  const document = getDictionary(locale).legal.security;
  return { title: `${document.title} | Gerpy ERP`, description: document.metadataDescription };
}

export default async function SecurityPage({ params }: LegalPageParams) {
  const { locale } = await params;
  if (!isLocale(locale)) notFound();
  const legal = getDictionary(locale).legal;
  return <LegalPage locale={locale} document={legal.security} lastUpdatedLabel={legal.common.lastUpdated} />;
}
